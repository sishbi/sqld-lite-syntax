...
/**
 * The alter statements whose exposed table this thread is already building.
 *
 * Identity, not equality: two statements holding the same text are different links in a migration
 * chain, and only a statement reached from itself is a cycle.
 */
private val alterStatementsInProgress: ThreadLocal<MutableSet<SqlAlterTableStmt>> =
...
private constructor(stub: AlterTableStmtStub?, nodeType: IElementType?, node: ASTNode?) :
  ...
  override fun tableExposed(): LazyQuery {
    return LazyQuery(
      ...
      query = result@{
          // Two files that both alter the same table and both have a null order see each other in
          // the schema, so asking one for its columns asks the other, which asks back. Nothing
          // else stops that: every call here builds a new LazyQuery, so the lazy cache never sees
          // the same instance twice, and the recursion kills the IDE with a StackOverflowError.
          // The annotator reports the real error, that an ALTER TABLE outside a migration file is
          // forbidden, so an empty result is all this has to return.
          val inProgress = alterStatementsInProgress.get()
