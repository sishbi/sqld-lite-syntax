...
private class TestParserDefinition(private val predefinedTables: Lazy<List<SqlFileBase>>) :
  ...
  companion object {
    // A stub file element type, because SqlParserDefinition requires one: a plain
    // IFileElementType contributes nothing to SchemaContributorIndex. The plain variant, not the
    // light one: it is what a real consumer registers, and the light stub builder it would bring
    // is never reached in a CoreApplicationEnvironment, where the environment module stands in for
    // the stub index.
    val FILE = IStubFileElementType<PsiFileStub<TestFile>>(TestLanguage)
  }
}
