 ...
 */
class SqldLiteCallHierarchyProvider : com.intellij.ide.hierarchy.HierarchyProvider {
     ...
     */
    private fun labelOf(element: PsiElement): SqldLiteStmtIdentifierMixin? =
        (
            PsiTreeUtil.getParentOfType(element, SqldLiteStmtIdentifierMixin::class.java, false)
                ?: SqldLiteQuery.containing(element)?.label
            )?.takeIf { it.name != null }

    override fun createHierarchyBrowser(target: PsiElement): HierarchyBrowser =
        SqldLiteCallHierarchyBrowser(target.project, target)

    override fun browserActivated(hierarchyBrowser: HierarchyBrowser) {
        (hierarchyBrowser as SqldLiteCallHierarchyBrowser)
