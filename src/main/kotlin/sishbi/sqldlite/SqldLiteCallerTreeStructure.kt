 ...
 */
class SqldLiteCallerTreeStructure(project: Project, label: PsiElement) :
    HierarchyTreeStructure(project, SqldLiteCallHierarchyNodeDescriptor(project, null, label, true)) {

    override fun buildChildren(descriptor: HierarchyNodeDescriptor): Array<Any> =
        when (val element = descriptor.psiElement) {
            is SqldLiteStmtIdentifierMixin -> callersOfQuery(element)
            is KtNamedFunction -> callersOfFunction(element)
            else -> emptyList()
        }.map { SqldLiteCallHierarchyNodeDescriptor(myProject, descriptor, it, false) }
            .toTypedArray()

    private fun callersOfQuery(label: SqldLiteStmtIdentifierMixin): List<PsiElement> =
        SqldLiteQueryCallSites.of(label).map { enclosingFunctionOr(it) }.distinct()
