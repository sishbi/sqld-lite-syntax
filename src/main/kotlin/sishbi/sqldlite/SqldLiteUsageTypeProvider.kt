...
import sishbi.sqldlite.psi.SqldLiteImportStmt

private val QUERY = UsageType { SqldLiteMessageBundle.message("usages.group.query") }
private val BIND_ARGUMENT = UsageType { SqldLiteMessageBundle.message("usages.group.bind.argument") }
private val QUERY_CALL = UsageType { SqldLiteMessageBundle.message("usages.group.query.call") }
private val QUERY_ARGUMENT = UsageType { SqldLiteMessageBundle.message("usages.group.query.argument") }
private val TYPE_IMPORT = UsageType { SqldLiteMessageBundle.message("usages.group.type.import") }
...
class SqldLiteUsageTypeProvider : UsageTypeProvider {
    ...
    private fun sqUsageType(element: PsiElement): UsageType? =
        when {
            PsiTreeUtil.getParentOfType(element, SqldLiteImportStmt::class.java, false) != null ->
                TYPE_IMPORT

            PsiTreeUtil.getParentOfType(element, SqldLiteJavaTypeMixin::class.java, false) != null ->
                COLUMN_TYPE

            PsiTreeUtil.getParentOfType(element, SqlBindParameter::class.java, false) != null ->
                BIND_ARGUMENT

            PsiTreeUtil.getParentOfType(element, SqldLiteStmtIdentifierMixin::class.java, false) != null ->
                QUERY
    ...
    private fun isGeneratedCall(element: PsiElement?) =
        (element as? KtCallExpression)?.let { SqldLiteQueryCall.forCall(it) } != null
}
