...
import sishbi.sqldlite.psi.impl.SqldLiteCreateIndexStmtImpl
 ...
 */
class SqldLiteCreateIndexElementType(name: String) : CreateIndexElementType(name) {
    override fun getExternalId() = "$EXTERNAL_ID_PREFIX$this"

    override fun createPsi(stub: SchemaContributorStub) = SqldLiteCreateIndexStmtImpl(stub, this)

    companion object {
        /** Must equal the `externalIdPrefix` of the `SqldLiteTypes` stubElementTypeHolder. */
        const val EXTERNAL_ID_PREFIX = "SqldLite.OVERLAY"
    }
}
