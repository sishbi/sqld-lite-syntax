...
object SqldLiteTextAttributes {
    ...
    /** A query label, such as `selectAll:`. Applied from the PSI. See [SqldLiteSemanticHighlights]. */
    val QUERY_LABEL = key("QUERY_LABEL", DefaultLanguageHighlighterColors.FUNCTION_DECLARATION)

    /** A bind argument, `?` or `:name`. Applied from the PSI. See [SqldLiteSemanticHighlights]. */
    ...
    private fun key(name: String, fallback: TextAttributesKey) =
        TextAttributesKey.createTextAttributesKey("SQLD_LITE.$name", fallback)
}
