 ...
 */
class SqldLiteFindUsagesProvider : FindUsagesProvider {

    /**
     * Indexes an identifier as code rather than as plain text, so a search for a Kotlin class finds
     * [SqldLiteJavaTypeReference] at the `import` and at the `AS` site. Without a scanner the
    ...
    override fun getType(element: PsiElement) =
        when (element) {
            is SqldLiteBindParameterMixin -> "usages.type.bind.argument"
        ...
        }.let { SqldLiteMessageBundle.message(it) }
