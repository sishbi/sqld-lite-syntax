 ...
 * [SqldLiteStmtIdentifierMixin] gives.
 *
 * Extends [SqlBindParameterImpl] because a mixin replaces the rule's `extends`, and the parent's own
 * PSI code expects that class.
 *
 * Must be `open`: the generated `SqldLiteBindParameterImpl` is Java and extends this class.
 */
open class SqldLiteBindParameterMixin(node: ASTNode) :
    ...
    /** Titles the target of a Find Usages search, for the reason [SqldLiteStmtIdentifierMixin] gives. */
    override fun getPresentation(): ItemPresentation =
        PresentationData(name.orEmpty(), containingFile.name, getIcon(0), null)

    /** Always fails, for the reason [SqldLiteStmtIdentifierMixin.setName] gives. */
