 ...
 */
class SqdCodeQuery private constructor(
    val label: SqdCodeStmtIdentifierMixin,
    ...
    companion object {
        ...
        private fun PsiElement.precedingLabel(): SqdCodeStmtIdentifierMixin? =
            generateSequence(prevSibling) { it.prevSibling }
                .filterIsInstance<SqdCodeStmtIdentifierMixin>()
                .firstOrNull()

        /** Everything between [this] label and the next one, which is the query's own text. */
        private fun SqdCodeStmtIdentifierMixin.statementsUnderLabel(): List<PsiElement> =
