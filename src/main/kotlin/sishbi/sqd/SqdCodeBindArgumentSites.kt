 ...
 * argument, the way [SqdCodeQueryCallSites] makes them agree on a label's call sites.
 */
object SqdCodeBindArgumentSites {

    /**
     * The value each call passes for [parameter]. Empty when the query mixes in a positional `?`
     * argument, for the reason [SqdCodeQuery.positionOf] gives.
     *
    ...
    fun of(parameter: SqlBindParameter): List<PsiElement> {
        val query = SqdCodeQuery.containing(parameter) ?: return emptyList()
        val position = query.positionOf(parameter) ?: return emptyList()

        return SqdCodeQueryCallSites.of(query.label).mapNotNull { argumentFor(it, query, position) }
    }
    ...
    fun usesOf(parameter: SqlBindParameter): List<PsiElement> {
        val siblings = SqdCodeQuery.containing(parameter)?.siblingsOf(parameter).orEmpty()
        return siblings + of(parameter)
    }

    /** The value [call] passes at [position], by argument name where the call names its arguments. */
    private fun argumentFor(call: KtCallExpression, query: SqdCodeQuery, position: Int): PsiElement? {
        val arguments = call.valueArguments
        val wanted = SqdCodeQuery.camelCase(query.parameterNames[position])

        val named = arguments.firstOrNull {
            it.getArgumentName()?.asName?.identifier?.let(SqdCodeQuery::camelCase) == wanted
        }
