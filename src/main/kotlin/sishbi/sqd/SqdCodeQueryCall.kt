 ...
 */
data class SqdCodeQueryCall(val queryName: String, val queriesFileBaseName: String?) {
     ...
     */
    fun preferred(matches: List<SqdCodeStmtIdentifierMixin>): List<SqdCodeStmtIdentifierMixin> {
        val fromNamedFile =
    ...
    companion object {
         ...
         * no nonsense target.
         */
        fun of(element: PsiElement): SqdCodeQueryCall? {
        ...
        fun forQueriesClass(queryName: String, queriesName: String): SqdCodeQueryCall? {
            if (!queriesName.endsWith(QUERIES_SUFFIX, ignoreCase = true)) return null
            return SqdCodeQueryCall(queryName, fileBaseNameOf(queriesName))
        }

        /** Null unless [call] is a call on a receiver whose name ends in `Queries`. */
        fun forCall(call: KtCallExpression): SqdCodeQueryCall? {
            ...
            return SqdCodeQueryCall(callee.getReferencedName(), fileBaseNameOf(receiverName))
        }
