 ...
 */
class SqdCodeGeneratedQueryUsageSearcher : CustomUsageSearcher() {
    ...
    private fun bindArgumentsFor(parameter: KtParameter): List<PsiElement> {
        ...
        return call
            .preferred(labels)
            .mapNotNull { SqdCodeQuery.of(it).parameterFor(parameter.name, position) }
    }
    ...
    ): Pair<SqdCodeQueryCall, List<SqdCodeStmtIdentifierMixin>>? {
        val queryName = function.name ?: return null
        val queriesName = function.containingClassOrObject?.name ?: return null
        val call = SqdCodeQueryCall.forQueriesClass(queryName, queriesName) ?: return null

        val project = function.project
        val labels = SqdCodeLabels.find(project, queryName, GlobalSearchScope.projectScope(project))
