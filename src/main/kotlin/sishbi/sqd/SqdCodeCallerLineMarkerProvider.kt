 ...
 */
class SqdCodeCallerLineMarkerProvider : LineMarkerProviderDescriptor() {

    override fun getName() = SqdCodeMessageBundle.message("gutter.callers.name")
    ...
    override fun collectSlowLineMarkers(
        ...
        if (file !is SqdCodeFile || DumbService.isDumb(file.project)) return
    ...
    private fun markerFor(element: PsiElement): LineMarkerInfo<*>? {
        ...
        val callers = SqdCodeQueryCallSites.of(label)
        return NavigationGutterIconBuilder
            .create(AllIcons.Actions.Find)
            .setTargets(callers)
            .setTooltipText(tooltipFor(callers.size))
            .setEmptyPopupText(SqdCodeMessageBundle.message("gutter.callers.none"))
            .createLineMarkerInfo(element)
    }

    private fun tooltipFor(count: Int) =
        if (count == 0) {
            SqdCodeMessageBundle.message("gutter.callers.none")
        } else {
            SqdCodeMessageBundle.message("gutter.callers.count", count)
        }
     ...
     */
    private fun SqdCodeStmtIdentifierMixin.identifierLeaf(): PsiElement? =
