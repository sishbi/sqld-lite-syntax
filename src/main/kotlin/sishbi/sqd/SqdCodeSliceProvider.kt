 ...
 * Kotlin call site passes for it. See [SqdCodeSliceUsage].
 *
 ...
 */
class SqdCodeSliceProvider : SliceLanguageSupportProvider {

    override fun createRootUsage(element: PsiElement, params: SliceAnalysisParams): SliceUsage =
        SqdCodeSliceUsage(element, params)
    ...
    override fun getExpressionAtCaret(atCaret: PsiElement, dataFlowToThis: Boolean): PsiElement? =
        if (dataFlowToThis) {
            PsiTreeUtil.getParentOfType(atCaret, SqdCodeBindParameterMixin::class.java)
        } else {
            null
        }
    ...
    override fun getRenderer(): SliceUsageCellRendererBase = SqdCodeSliceRenderer()
...
private class SqdCodeSliceUsage(
    ...
    override fun processUsagesFlownDownTo(
        ...
        val parameter = element as? SqdCodeBindParameterMixin ?: return
        SqdCodeBindArgumentSites.of(parameter).forEach { argument ->
    ...
    override fun copy() = SqdCodeSliceUsage(source, params)
}

private class SqdCodeSliceRenderer : SliceUsageCellRendererBase() {

    override fun customizeCellRendererFor(usage: SliceUsage) {
        val label = (usage as? SqdCodeSliceUsage)?.label ?: return
