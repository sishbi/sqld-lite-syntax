 ...
 *
 * Uses the same word-index scan as navigation ([SqdCodeQueryCallSites]), not a nested Find Usages
 * run, which starts its own indexing and progress work for every label while the user types.
 */
class SqdCodeUnusedQueryInspection : LocalInspectionTool() {

    override fun buildVisitor(holder: ProblemsHolder, isOnTheFly: Boolean): PsiElementVisitor {
        if (holder.file !is SqdCodeFile) return PsiElementVisitor.EMPTY_VISITOR
        if (DumbService.isDumb(holder.project)) return PsiElementVisitor.EMPTY_VISITOR

        return object : PsiElementVisitor() {
            override fun visitElement(element: PsiElement) {
                if (element !is SqdCodeStmtIdentifierMixin) return
                if (element.name == null) return
                if (SqdCodeQueryCallSites.any(element)) return

                holder.registerProblem(
                    element,
                    SqdCodeMessageBundle.message("inspection.unused.query.message"),
    ...
    class DeleteQueryFix(label: SqdCodeStmtIdentifierMixin) :
        LocalQuickFixOnPsiElement(label) {

        override fun getFamilyName() = SqdCodeMessageBundle.message("inspection.unused.query.fix")
        ...
        override fun invoke(
            ...
            val last = generateSequence(startElement.nextSibling) { it.nextSibling }
                .takeWhile { it !is SqdCodeStmtIdentifierMixin }
                .lastOrNull()
                ?: startElement
