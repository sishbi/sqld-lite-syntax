 ...
 */
class SqdCodeFindUsagesHandlerFactory : FindUsagesHandlerFactory() {
     ...
     * and only the set of starting points was wrong. See [SqdCodeSchemaChain].
     */
        ...
        object : FindUsagesHandler(element) {
            override fun processElementUsages(
                ...
                val chain = ApplicationManager.getApplication().runReadAction(
                    Computable { SqdCodeSchemaChain.of(element as NamedElement) },
                )
    ...
    private fun usesOf(element: PsiElement): List<PsiElement> =
        when (element) {
            is SqdCodeStmtIdentifierMixin -> SqdCodeQueryCallSites.of(element)
            is SqdCodeBindParameterMixin -> SqdCodeBindArgumentSites.usesOf(element)
            else -> emptyList()
        }
}
