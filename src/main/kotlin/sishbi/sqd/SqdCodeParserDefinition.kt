 ...
 */
object SqdCodeFileElementType : IStubFileElementType<PsiFileStub<SqdCodeFile>>(SqdCodeLanguage) {
    override fun getExternalId() = "SqdCode.FILE"

    /** Bump when a change here alters what the stubs hold, so the IDE reindexes. */
    override fun getStubVersion() = 1
}

val SQD_CODE_FILE = SqdCodeFileElementType
 ...
 */
class SqdCodeParserDefinition : SqlParserDefinition() {
    override fun getFileNodeType() = SQD_CODE_FILE

    override fun getLanguage() = SqdCodeLanguage

    override fun createFile(viewProvider: FileViewProvider) = SqdCodeFile(viewProvider)
...
private fun installParserOverride() {
    ...
    synchronized(installLock) {
        if (SqlParserUtil.stmt_list != null && SqlParserUtil.bind_parameter != null) return
        SqdCodeParserUtil.overrideSqlParser()
    }
}
