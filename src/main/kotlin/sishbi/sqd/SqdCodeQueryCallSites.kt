...
object SqdCodeQueryCallSites {
     ...
     */
    fun of(label: SqdCodeStmtIdentifierMixin): List<KtCallExpression> =
        ApplicationManager.getApplication().runReadAction(Computable { search(label) })

    /** True when [label] has at least one call. */
    fun any(label: SqdCodeStmtIdentifierMixin): Boolean = of(label).isNotEmpty()

    private fun search(label: SqdCodeStmtIdentifierMixin): List<KtCallExpression> {
    ...
    private fun callsIn(candidate: PsiElement, name: String, fileBaseName: String): List<KtCallExpression> =
        listOfNotNull(candidate, candidate.firstChild)
            .filter { at ->
                val call = SqdCodeQueryCall.of(at) ?: return@filter false
                call.queryName == name &&
