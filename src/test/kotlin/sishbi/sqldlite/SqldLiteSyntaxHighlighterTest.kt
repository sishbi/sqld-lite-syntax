...
class SqldLiteSyntaxHighlighterTest : ParsingTestCase("", "sq", SqldLiteParserDefinition()) {
    ...
    fun testColoursEachClassOfToken() {
        assertKey(SqldLiteTextAttributes.KEYWORD, SqlTypes.SELECT)
        assertKey(SqldLiteTextAttributes.KEYWORD, SqlTypes.WHERE)
        assertKey(SqldLiteTextAttributes.IDENTIFIER, SqlTypes.ID)
        assertKey(SqldLiteTextAttributes.NUMBER, SqlTypes.DIGIT)
        assertKey(SqldLiteTextAttributes.STRING, SqlTypes.STRING)
        assertKey(SqldLiteTextAttributes.OPERATOR, SqlTypes.EQ)
        assertKey(SqldLiteTextAttributes.OPERATOR, SqlTypes.CONCAT)
        assertKey(SqldLiteTextAttributes.PARENTHESES, SqlTypes.LP)
        assertKey(SqldLiteTextAttributes.DOT, SqlTypes.DOT)
        assertKey(SqldLiteTextAttributes.COMMA, SqlTypes.COMMA)
        assertKey(SqldLiteTextAttributes.SEMICOLON, SqlTypes.SEMI)
        assertKey(SqldLiteTextAttributes.LINE_COMMENT, SqlTypes.COMMENT)
        assertKey(SqldLiteTextAttributes.DOC_COMMENT, SqlTypes.JAVADOC)
    }
    ...
    fun testLeavesWhiteSpaceAndBadCharactersAlone() {
        ...
        // character. SqldLiteHighlightAnnotator colours those from the PSI, where a real bind
