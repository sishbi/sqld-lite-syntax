...
class SqldLiteQueryUsageTest : SqldLitePlatformTestCase() {
    ...
    fun testGreysOutOnlyTheQueriesNoKotlinCalls() {
        ...
        myFixture.enableInspections(SqldLiteUnusedQueryInspection())
    ...
    fun testReportsTheKotlinCallAsAUsage() {
        ...
        val factory = SqldLiteFindUsagesHandlerFactory()
    ...
    fun testOffersTheLabelAsASearchTarget() {
        ...
        assertEquals("findByMemberId", (target as? SqldLiteStmtIdentifierMixin)?.name)
        assertEquals(
            AllIcons.Nodes.Method,
            (target as? SqldLiteStmtIdentifierMixin)?.presentation?.getIcon(true),
        )
    }
    ...
    fun testOffersABindArgumentAsASearchTarget() {
        ...
        assertEquals("loan_id", (target as? SqldLiteBindParameterMixin)?.name)
        assertEquals(
            AllIcons.Nodes.Parameter,
            (target as? SqldLiteBindParameterMixin)?.presentation?.getIcon(true),
        )
    }
    ...
    fun testReportsTheBindArgumentAsAUsageOfTheGeneratedParameter() {
        ...
        SqldLiteGeneratedQueryUsageSearcher()
    ...
    fun testReportsEveryOtherMentionOfTheSameBindArgument() {
        ...
        val handler = requireNotNull(
            SqldLiteFindUsagesHandlerFactory().createFindUsagesHandler(parameter, false),
        )
        handler.processElementUsages(parameter, usages, handler.findUsagesOptions)
    ...
    fun testNamesTheGroupEveryUsageAppearsUnder() {
        ...
        myFixture.copyFileToProject("BookLoans.sq")
        val file = myFixture.configureFromTempProjectFile("BookLoans.sq")
        val provider = SqldLiteUsageTypeProvider()
    ...
    fun testRegistersTheSliceProviderTheDataFlowTabAsksFor() {
        ...
        assertInstanceOf(provider, SqldLiteSliceProvider::class.java)
     ...
     * [SqldLitePlatformTestCase] turns that into one.
     */
    fun testTouchesNoPsiWithoutAReadAction() {
        ...
        val label = findByMemberIdLabel()
        val handler = requireNotNull(
            SqldLiteFindUsagesHandlerFactory().createFindUsagesHandler(label, false),
        )
        val params = SliceAnalysisParams().apply { scope = AnalysisScope(project) }
        val renderer = SqldLiteSliceProvider().renderer
        val usage = SqldLiteSliceProvider().createRootUsage(label, params)
    ...
    fun testFlowsABindArgumentBackToTheValueEachCallPasses() {
        ...
        assertEquals(
            parameter,
            SqldLiteSliceProvider().getExpressionAtCaret(caret, dataFlowToThis = true),
        )
        assertNull(SqldLiteSliceProvider().getExpressionAtCaret(caret, dataFlowToThis = false))
    ...
    fun testBuildsTheCallHierarchyOfAQuery() {
        ...
        val structure = SqldLiteCallerTreeStructure(project, label)
    ...
    fun testFindsTheQueryUnderTheCaretForTheHierarchyWindow() {
        ...
        val target = SqldLiteCallHierarchyProvider().getTarget(context)
    ...
    fun testTakesABindArgumentToBeAQuestionAboutItsQuery() {
        ...
        assertEquals("updateStatus", (target as? SqldLiteStmtIdentifierMixin)?.name)
    }
    ...
    fun testHasNoHierarchyForAnUnlabelledStatement() {
        ...
        assertNull(SqldLiteCallHierarchyProvider().getTarget(context))
    }
    ...
    private fun inflowOf(element: PsiElement): List<String> {
        ...
        val root = SqldLiteSliceProvider().createRootUsage(element, params)
    ...
    private fun targets(element: PsiElement, offset: Int): List<PsiElement> =
        SqldLiteSqGotoDeclarationHandler()
            .getGotoDeclarationTargets(element, offset, myFixture.editor)
            ?.toList()
            .orEmpty()

    private fun findByMemberIdLabel(): SqldLiteStmtIdentifierMixin {
        myFixture.copyFileToProject("BookLoans.sq")
        val file = myFixture.configureFromTempProjectFile("BookLoans.sq")
        return PsiTreeUtil
            .findChildrenOfType(file, SqldLiteStmtIdentifierMixin::class.java)
