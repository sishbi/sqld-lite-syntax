 ...
 * platform can supply. [SqldLiteSemanticHighlights] holds the decision, so the decision is what is
 * tested.
 */
class SqldLiteSemanticHighlightsTest : ParsingTestCase("", "sq", SqldLiteParserDefinition()) {
    ...
    private fun highlights(file: PsiFile): List<Pair<String, TextAttributesKey>> =
        PsiTreeUtil.findChildrenOfAnyType(file, true, com.intellij.psi.PsiElement::class.java)
            .flatMap { element -> SqldLiteSemanticHighlights.of(element).map { it } }
            .sortedBy { it.first.startOffset }
            .map { (range, key) -> file.text.substring(range.startOffset, range.endOffset) to key }

    private companion object {
        val QUERY_LABEL = SqldLiteTextAttributes.QUERY_LABEL
        val BIND_PARAMETER = SqldLiteTextAttributes.BIND_PARAMETER
    }
}
