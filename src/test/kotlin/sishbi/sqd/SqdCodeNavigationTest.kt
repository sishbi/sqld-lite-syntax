 ...
 */
class SqdCodeNavigationTest : SqdCodePlatformTestCase() {
    ...
    fun testNavigatesToTheLabelInTheFileTheReceiverNames() {
        ...
        assertEquals("findByMemberId", (targets.single() as SqdCodeStmtIdentifierMixin).name)
    ...
    fun testNavigatesThroughADatabaseProperty() {
        ...
        assertEquals("updateStatus", (targets.single() as SqdCodeStmtIdentifierMixin).name)
    }
    ...
    private fun labels(name: String) =
        SqdCodeLabels.find(project, name, GlobalSearchScope.projectScope(project))
    ...
    private fun targetsAtCaret(call: String): List<com.intellij.psi.PsiElement> {
        ...
        return SqdCodeGotoDeclarationHandler()
