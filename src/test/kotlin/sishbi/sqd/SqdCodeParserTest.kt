 ...
 */
class SqdCodeParserTest : ParsingTestCase("", "sq", SqdCodeParserDefinition()) {
    ...
    fun testParsesMigrationFileWithNoQueryLabel() {
        val file = createPsiFile("Migration", loadFile("Migration.sqm"))
    ...
    private fun labels(file: PsiFile) =
        PsiTreeUtil.findChildrenOfType(file, SqdCodeStmtIdentifierMixin::class.java)
            .filter { it.name != null }
